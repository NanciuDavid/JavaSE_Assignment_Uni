...
javac -cp src/main/java -d target/classes src/main/java/org/example/ParallelMatrixAdditionBenchmark.java
...
if [ $? -eq 0 ]; then
    ...
    java -cp target/classes org.example.ParallelMatrixAdditionBenchmark
else
